...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "tic-80" || "$var" == "all" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$tic80_patches" ]]; then
      ...
      cmake -DCMAKE_BUILD_TYPE=MinSizeRel -DBUILD_PLAYER=OFF -DBUILD_SOKOL=OFF -DBUILD_SDL=OFF -DBUILD_DEMO_CARTS=OFF -DBUILD_LIBRETRO=ON .
	  make clean
	  make -j$(nproc)
	  ...
	  echo " "
	  echo "tic80_libretro.so has been created and has been placed in the cores64 subfolder"
	fi
