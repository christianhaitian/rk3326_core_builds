...
do
	...
	if [[ "$var" == "dosbox_pure" || "$var" == "all" ]] && [[ "$(getconf LONG_BIT)" == "64" ]]; then
	 ...
	 if [[ ! -z "$dosboxpure_patches" ]]; then
	  ...
	  echo $gitcommit > ../cores$(getconf LONG_BIT)/dosbox_pure_libretro.so.commit
	...
	if [[ "$var" == "mgba" || "$var" == "all" ]] && [[ "$(getconf LONG_BIT)" == "64" ]]; then
	 ...
	 if [[ ! -z "$mgba_patches" ]]; then
	  ...
	  gitcommit=$(git show | grep commit | cut -c -14 | cut -c 8-)
	  echo $gitcommit > ../cores$(getconf LONG_BIT)/mgba_rumble_libretro.so.commit
	...
	if [[ "$var" == "sameboy" || "$var" == "all" ]] && [[ "$(getconf LONG_BIT)" == "64" ]]; then
	 cd $cur_wd
	  if [ ! -d "sameboy/" ]; then
	...
	if [[ "$var" == "gpsp" || "$var" == "all" ]]; then
	 ...
	 if [[ ! -z "$gpsp_patches" ]]; then
	  ...
	  cp gpsp_libretro.so ../cores$(getconf LONG_BIT)/.

	  gitcommit=$(git show | grep commit | cut -c -14 | cut -c 8-)
	  echo $gitcommit > ../cores$(getconf LONG_BIT)/$(basename $PWD)_libretro.so.commit

	  echo " "
	  echo "gpsp_libretro.so has been created and has been placed in the rk3326_core_builds/cores$(getconf LONG_BIT) subfolder"
	fi
	  ...
	  if [[ $flycast_rumblepatch == "yes" ]]; then
		...
		do
		  ...
		  if [[ "$(getconf LONG_BIT)" == "64" ]]; then
	        ...
	        gitcommit=$(git show | grep commit | cut -c -14 | cut -c 8-)
       	    echo $gitcommit > ../cores$(getconf LONG_BIT)/flycast_rumble_libretro.so.commit
		  else
	        ...
	        gitcommit=$(git show | grep commit | cut -c -14 | cut -c 8-)
	        echo $gitcommit > ../cores$(getconf LONG_BIT)/flycast32_rumble_libretro.so.commit
		  fi
		done
	  fi
	 ...
	 if [[ ! -z "$pcsx_rearmed_patches" ]]; then
	  ...
	  if [[ $pcsx_rearmed_rumblepatch == "yes" ]]; then
		...
		do
	      ...
	      gitcommit=$(git show | grep commit | cut -c -14 | cut -c 8-)
	      echo $gitcommit > ../cores$(getconf LONG_BIT)/pcsx_rearmed_rumble_libretro.so.commit
		done
	  fi
	...
	if [[ "$var" == "parallel-n64" || "$var" == "all" ]]; then
	 ...
	 if [[ ! -z "$paralleln64_patches" ]]; then
	  ...
	  gitcommit=$(git show | grep commit | cut -c -14 | cut -c 8-)
	  echo $gitcommit > ../cores$(getconf LONG_BIT)/parallel_n64_libretro.so.commit
