...
cp -f -v /usr/local/bin/"MSDOS - Hide zip games.sh" /opt/system/"MSDOS - Hide zip games.sh"
rm -f -v /opt/system/"MSDOS - Show zip games.sh"
printf "MSDOS has been set to show games with .zip extensions.\nEmulationstation will now be restarted." >> /dev/tty1
sleep 5
printf "\033c" >> /dev/tty1
